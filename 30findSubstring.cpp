...
class Solution {
public:
    bool test(string s,vector<string>& words) {
        vector<int> bitmap(words.size(),0);
        int wordLength = words[0].size();
        int size = 0;
        int wordsSize = words.size();
        int sLength = s.size();
        for(int i=0;i < sLength;i += wordLength) {
            int lastSize = size;
            for(int j=0;j < wordsSize;j++) {
                if (s.compare(i,wordLength,words[j])==0&&bitmap[j]==0) {
                    bitmap[j] = 1;
                    size++;
                    break;
                }
    ...
    vector<int> findSubstring(string s, vector<string>& words) {
        ...
        for(int i=0;i <= slength - length;i++) {
            bool res = test(s.substr(i,length),words);
            if (res == true) {
                ans.push_back(i);
            }
        }
